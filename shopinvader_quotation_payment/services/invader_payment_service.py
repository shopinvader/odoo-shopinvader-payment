...
class InvaderPaymentService(Component):
    ...
    def _invader_find_payable_from_target(self, target, **params):
        if target == "quotation":
            ...
            return quotation
        return super(InvaderPaymentService, self)._invader_find_payable_from_target(target, **params)

    def _invader_get_target_validator(self):
        res = super(InvaderPaymentService, self)._invader_get_target_validator()
